...
import { setDiscordRequestContext } from './discordTools.js';
...
export async function deliverCheckins(client: Client): Promise<number> {
  // Unprompted turn: no human requester, so clear the owner-gate context. This
  // marks tools like start_project as "MrRoboto acting on his own" (allowed).
...
export async function postBriefing(client: Client): Promise<boolean> {
  setDiscordRequestContext({}); // unprompted turn — see deliverCheckins
