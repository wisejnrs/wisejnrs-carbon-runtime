...
export async function runGroundedChat(
  ...
  owner = true,
  opts: { speaker?: string; conversationContext?: string; channelName?: string } = {},
): Promise<GroundedReply> {
  const display = createProgressDisplay(update);
  const onProgress: ChatProgress = display.onNote;

  // In multi-party channels attribute the turn to whoever spoke, so the model can
  // tell participants apart (e.g. Mike vs Paul) instead of collapsing them to "User".
  const userContent = opts.speaker ? `${opts.speaker}: ${question}` : question;
  ...
  try {
    ...
    const eventContext = events.length
      ...
      : '';
    // Recent channel back-and-forth (incl. turns the bot wasn't tagged in), so a
    // mention lands with full awareness of who said what just before it.
    const convoContext = opts.conversationContext
      ? '\n\n' + untrustedBlock('recent-conversation', opts.conversationContext)
      : '';
    // Tell the model where it already is, so "this channel" / "here" / "the current
    // channel" resolve and it doesn't go hunting for a channel it's already in.
    const whereContext = opts.channelName
      ? `\n\nYou are currently in the #${opts.channelName} Discord channel — this message came from there. ` +
        `When the user says "this channel", "here", or "the current channel", they mean #${opts.channelName}, ` +
        `and you are already present in it (your reply is posted straight back to #${opts.channelName}). ` +
        `To invite someone into #${opts.channelName}, use discord_invite with no channel argument.`
      : '';
